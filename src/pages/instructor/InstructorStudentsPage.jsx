...
export default function InstructorStudentsPage() {
  ...
  const courseOptions = useMemo(() => {
    ...
    students.forEach((student) => {
      const label = courseLabel(student);
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((student) => {
      const label = courseLabel(student);
      if (courseFilter !== "all" && label !== courseFilter) return false;
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#0a1628" }}>
                  {["Student", "Student ID", "Course Section", "Department", "Status", "Last Login"].map((header) => (
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={`${student.id}-${student.courseId}-${student.sectionNumber}`} style={{ borderTop: `1px solid ${border}` }}>
                    ...
                    <td style={{ padding: "13px 16px", color: accent, fontSize: 13, fontWeight: 700 }}>{courseLabel(student) || "-"}</td>
